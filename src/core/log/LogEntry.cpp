...
std::u8string LogEntry::format_log(void) const {
    const std::string time = std::format("[{:%F %T}]", this->time); // https://cpprefjp.github.io/reference/chrono/format.html
    const std::u8string time_stamp(time.begin(), time.end()); // ASCII前提なので仮にASCIIを返さなかった場合はバグるので要修正
    ...
    return time_stamp
        ...
        + log_level
        + u8" "
        + add_indent(this->message, indent);
}