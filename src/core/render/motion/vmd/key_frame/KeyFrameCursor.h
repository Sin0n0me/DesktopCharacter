...
protected:
    const std::weak_ptr<KeyFrameTimer> frame_manager;
...
public:
    ...
    std::optional<std::reference_wrapper<const T>> get_previous_key_frame(void) const;
    std::optional<std::reference_wrapper<const T>> get_next_key_frame(void) const;
    std::optional<std::reference_wrapper<const T>> get_current_key_frame(void) const;
    std::optional<std::reference_wrapper<const T>> get_first_key_frame(void) const;
    std::optional<std::reference_wrapper<const T>> get_last_key_frame(void) const;
};
...
inline std::vector<T> KeyFrameCursor<T>::sort(
    ...
    // これなんとかしたい(Tのconstフィールド対策でこうなっている)
...
inline KeyFrameCursor<T>::KeyFrameCursor(
    ...
    key_frame_index(0) {
    if(const auto manager = this->frame_manager.lock()) {
        manager->join(this);
    }
}
...
inline void KeyFrameCursor<T>::on_update_key_frame(void) {
    ...
    const auto& current_frame = manager->get_current_frame();
...
template<typename T>
inline std::optional<std::reference_wrapper<const T>> KeyFrameCursor<T>::get_previous_key_frame(void) const {
    if(this->key_frame_list.empty()) {
        return std::nullopt;
    }
    ...
    return this->key_frame_list.at(
        static_cast<size_t>(this->key_frame_index - 1)
    );
}

template<typename T>
inline std::optional<std::reference_wrapper<const T>> KeyFrameCursor<T>::get_next_key_frame(void) const {
    if(this->key_frame_list.empty()) {
        return std::nullopt;
    }

    const auto next = static_cast<size_t>(this->key_frame_index) + 1;
    if(this->key_frame_list.size() < next + 1) {
        return std::nullopt;
    }

    return this->key_frame_list.at(next);
}

template<typename T>
inline std::optional<std::reference_wrapper<const T>> KeyFrameCursor<T>::get_current_key_frame(void) const {
    if(this->key_frame_list.empty()) {
        return std::nullopt;
    }
    return this->key_frame_list.at(
...
template<typename T>
inline std::optional<std::reference_wrapper<const T>> KeyFrameCursor<T>::get_first_key_frame(void) const {
    if(this->key_frame_list.empty()) {
        return std::nullopt;
    }
...
template<typename T>
inline std::optional<std::reference_wrapper<const T>> KeyFrameCursor<T>::get_last_key_frame(void) const {
    if(this->key_frame_list.empty()) {
        return std::nullopt;
    }
